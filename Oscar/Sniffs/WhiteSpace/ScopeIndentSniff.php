 ...
 *
 * Fork of Generic.WhiteSpace.ScopeIndent: when closing a `match` body, do not use
 * `nested_parenthesis` or `conditions` from outer constructs (e.g. a chained
 * `->filter(function () { ... })`). Those anchors pull the baseline back one level
 * and under-indent the next statements (see tests/fixtures/valid/Report.php).
 *
...
namespace Oscar\Sniffs\WhiteSpace;
    ...
    public function process(File $phpcsFile, int $stackPtr)
    {
        ...
        for ($i = ($stackPtr + 1); $i < $phpcsFile->numTokens; $i++) {
            ...
            if ($checkAnnotations === true
                ...
                && $tokens[$i]['sniffCode'] === 'Oscar.WhiteSpace.ScopeIndent'
            ...
            */
            ...
            if (isset($tokens[$i]['scope_condition']) === true
                ...
                $isMatchScopeCloser = ($tokens[$tokens[$i]['scope_condition']]['code'] === T_MATCH);

                $parens = 0;
                if ($isMatchScopeCloser === false
